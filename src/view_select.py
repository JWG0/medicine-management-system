...
class SelectFrame(Frame):  # 继承Frame类
    ...
    def createPage(self):
        Label(self, text='查询药品').grid(row=1, column=0)
        self.row = 2
        ...
        Button(self, text='查询', command=self.command_select_drug).grid(row=self.row, column=8, padx=5)

        Label(self, text='查询员工').grid(row=4, column=0)
        self.row = 5
        ...
        Button(self, text='查询', command=self.command_select_employee).grid(row=self.row, column=8, padx=5)

        Label(self, text='查询供应商').grid(row=7, column=0)
        self.row = 8
        ...
        Button(self, text='查询', command=self.command_select_supplier).grid(row=self.row, column=8, padx=5)

        Label(self, text='查询客户信息').grid(row=9, column=0)
        self.row = 10
        ...
        Button(self, text='查询', command=self.command_select_customer).grid(row=self.row, column=8, padx=5)

        Label(self, text='查询库房信息').grid(row=11, column=0)
        self.row = 12
        ...
        Button(self, text='查询', command=self.command_select_warehouse).grid(row=self.row, column=8, padx=5)

        self.row = 14
        Button(self, text='查询供应关系', command=self.command_select_sr).grid(
            row=self.row, column=2, padx=5,ipady=8,pady=12)
        Button(self, text='查询存储关系', command=self.command_select_store_r).grid(
            row=self.row, column=4, padx=5,ipady=8)
        Button(self, text='查询销售记录', command=self.command_select_sell).grid(
            row=self.row, column=6, padx=5,ipady=8)



    def command_select_drug(self):
        if select_drug(self.drug_no.get(),self.drug_name.get(),
                       self.drug_level.get(),self.drug_price.get()):
            pass
        else:
            showinfo(title='错误', message='查询数据失败！')

    def command_select_employee(self):
        if select_employee(self.employee_no.get(),self.employee_name.get(),
    ...
    def command_select_supplier(self):
        if select_supplier(self.supplier_no.get(),self.supplier_name.get(),self.supplier_city.get()):
            pass
        else:
            showinfo(title='错误', message='查询数据失败！')

    def command_select_customer(self):
        if select_customer(self.customer_no.get(),self.customer_name.get(),self.customer_city.get()):
            pass
        else:
            showinfo(title='错误', message='查询数据失败！')

    def command_select_warehouse(self):
        if select_warehouse(self.warehouse_no.get(),self.warehouse_name.get(),
                            self.warehouse_employee_no.get(),self.warehouse_sum.get()):
            pass
        else:
            showinfo(title='错误', message='查询数据失败！')
    def command_select_sr(self):
        if select_sr():
            pass
        else:
            showinfo(title='错误', message='查询数据失败！')
    def command_select_store_r(self):
        if select_store_r():
            pass
        else:
            showinfo(title='错误', message='查询数据失败！')
    def command_select_sell(self):
        if select_sell():
            pass
        else:
            showinfo(title='错误', message='查询数据失败！')
