...
class InsertFrame(Frame):  # 继承Frame类
    ...
    def createPage(self):
        Label(self, text='添加药品').grid(row=1, column=0)
        self.row=2
        ...
        self.drug_price.set('15')
        Button(self, text='录入', command=self.command_insert_drug).grid(row=self.row, column=8,padx=5)

        Label(self, text='添加员工').grid(row=4, column=0, ipady=10)
        self.row = 5
        ...
        Button(self, text='录入', command=self.command_insert_employee).grid(row=self.row, column=8, padx=5)

        Label(self, text='添加供应商').grid(row=7, column=0)
        self.row = 8
        ...
        Button(self, text='录入', command=self.command_insert_supplier).grid(row=self.row, column=8, padx=5)

        Label(self, text='添加客户信息').grid(row=9, column=0)
        self.row = 10
        ...
        Button(self, text='录入', command=self.command_insert_customer).grid(row=self.row, column=8, padx=5)

        Label(self, text='添加库房信息').grid(row=11, column=0)
        self.row = 12
        ...
        Button(self, text='录入', command=self.command_insert_warehouse).grid(row=self.row, column=8, padx=5)

    def command_insert_drug(self):
        if insert_drug(self.drug_no.get(),self.drug_name.get(),
                       self.drug_level.get(),self.drug_price.get()):
            showinfo(title='提示', message='插入数据成功！')
        else:
            showinfo(title='错误', message='插入数据失败！')

    def command_insert_employee(self):
        if insert_employee(self.employee_no.get(),self.employee_name.get(),
    ...
    def command_insert_supplier(self):
        if insert_supplier(self.supplier_no.get(),self.supplier_name.get(),self.supplier_city.get()):
            showinfo(title='提示', message='插入数据成功！')
        else:
            showinfo(title='错误', message='插入数据失败！')

    def command_insert_customer(self):
        if insert_customer(self.customer_no.get(),self.customer_name.get(),self.customer_city.get()):
            showinfo(title='提示', message='插入数据成功！')
        else:
            showinfo(title='错误', message='插入数据失败！')

    def command_insert_warehouse(self):
        if insert_warehouse(self.warehouse_no.get(),self.warehouse_name.get(),self.warehouse_employee_no.get()):
            showinfo(title='提示', message='插入数据成功！')
        else:
            showinfo(title='错误', message='插入数据失败！')
